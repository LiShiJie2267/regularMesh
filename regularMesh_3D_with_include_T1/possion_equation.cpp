...
int main(int argc, char* argv[])
{
    ...
    int n_bas = tetrahedron_basis_function.size();
    std::vector<AFEPack::Point<3> > gv(4);
	...
	{       
		...
		{
			std::vector<AFEPack::Point<3> > point=tetrahedron_coord_transform.local_to_global(q_point, lv, gv);
			double Jxy=quad_info.weight(l)*tetrahedron_coord_transform.local_to_global_jacobian(q_point[l], lv, gv);
			for(index_iterator it=it_b2->begin();it!=it_b2->end();it++)
			{
				for(index_iterator it2=it_b2->begin();it2!=it_b2->end();it2++)	
					stiff_mat.add(it->second,it2->second,Jxy*innerProduct(tetrahedron_basis_function[it->first.second].gradient(point[l],gv),tetrahedron_basis_function[it2->first.second].gradient(point[l],gv)));
				rhs(it->second)+=Jxy*f(point[l])*tetrahedron_basis_function[it->first.second].value(point[l],gv);
			}
		}	
	}
